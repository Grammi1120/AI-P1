...
def uniformCostSearch(problem: SearchProblem):
    ...
    while not frontier.isEmpty():
        ...
        for x in problem.getSuccessors(node):
            s = x[0]
            cost = x[2]
            new_cost = problem.getCostOfActions(path + [x[1]])
            if s not in reached or new_cost < reached[s]:
                reached[s] = new_cost
                frontier.push((s, path + [x[1]]), new_cost)
    return []
